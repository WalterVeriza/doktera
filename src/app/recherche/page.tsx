    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          {avis.length > 2 && (
            <button onClick={() => setExpanded(!expanded)} style={{ marginTop: '10px', width: '100%', padding: '9px', borderRadius: '10px', background: expanded ? '#f0ece2' : '#faf8f4', border: `1px solid ${expanded ? '#e0dbd0' : '#f0ece2'}`, cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontSize: '0.78rem', fontWeight: 600, color: '#7a7260', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
...
function PanelClinique({ clinique, onClose }: any) {
  ...
  return (
    <>
      ...
      <div ref={panelRef} style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '420px', background: 'white', zIndex: 201, boxShadow: '-8px 0 40px rgba(13,43,34,0.2)', display: 'flex', flexDirection: 'column', fontFamily: 'Outfit, sans-serif', animation: 'slideIn 0.22s ease-out', overflowY: 'auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #0d2b22, #163d2f)', padding: '28px 24px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', position: 'relative' }}>
          <button onClick={onClose} style={{ position: 'absolute', top: '14px', right: '14px', width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>X</button>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'inline-block', marginTop: '6px', background: 'rgba(200,153,42,0.2)', color: '#e6b84a', fontSize: '0.65rem', fontWeight: 700, padding: '3px 10px', borderRadius: '50px', border: '1px solid rgba(200,153,42,0.2)' }}>
        ...
        <div style={{ flex: 1, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ background: '#faf8f4', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {clinique.adresse && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                <span>📍</span>
                <div>
                  <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#a8a090', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '2px' }}>Adresse</div>
                  <div style={{ fontSize: '0.85rem', color: '#1a1512' }}>{clinique.adresse}</div>
                  {clinique.region && <div style={{ fontSize: '0.75rem', color: '#7a7260' }}>{clinique.region}</div>}
                </div>
              </div>
            )}
            {clinique.telephone && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <span>📞</span>
                <div>
                  <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#a8a090', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '2px' }}>Telephone</div>
                  <div style={{ fontSize: '0.85rem', color: '#22816a', fontWeight: 600 }}>{clinique.telephone}</div>
                </div>
              </div>
            )}
            {clinique.email && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.85rem', color: '#22816a', fontWeight: 600 }}>{clinique.email}</div>
                </div>
              </div>
            )}
          </div>
          ...
          {clinique.description && (
            <div>
              <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#a8a090', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>A propos</div>
              <p style={{ fontSize: '0.83rem', color: '#4a4035', lineHeight: 1.65, margin: 0 }}>{clinique.description}</p>
            </div>
          )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {services.map((s: any) => (
                  <div key={s.id} style={{ background: '#faf8f4', borderRadius: '10px', padding: '10px 14px', border: '1px solid #f0ece2', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '0.82rem', fontWeight: 600, color: '#0d2b22' }}>{s.nom}</div>
                      <div style={{ fontSize: '0.7rem', color: '#7a7260' }}>
                        {CATEGORIES_CLINIQUE.find(c => c.id === s.categorie)?.label}
                        {s.tarif ? ' · ' + Number(s.tarif).toLocaleString() + ' Ar' : ''}
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid #f0ece2', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button
            onClick={() => window.open('/clinique/' + clinique.id, '_blank')}
          ...
          >
            Voir la page et reserver
          </button>
          <button onClick={onClose} style={{ padding: '11px', borderRadius: '10px', background: '#f0ece2', color: '#0d2b22', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', fontFamily: 'Outfit, sans-serif' }}>
            Fermer
          </button>
        </div>
      </div>
...
export default function RecherchePage() {
  ...
  const fetchAll = async (s = specialite, txt = search) => {
    ...
    const { data: cliniqueData } = await supabase
      .from('cliniques')
      .select('*, services:clinique_services(*)')
      .eq('plan_actif', true)  // ── FILTRE plan_actif ──────────────────────────
  ...
  const selectStyle: React.CSSProperties = { padding: '10px 14px', background: 'white', border: '1.5px solid #f0ece2', borderRadius: '10px', fontFamily: 'Outfit, sans-serif', fontSize: '0.85rem', color: '#1a1512', outline: 'none', cursor: 'pointer' }
  ...
  const renderPanel = () => {
    ...
    return (
      <>
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,43,34,0.3)', zIndex: 200, backdropFilter: 'blur(3px)' }} />
        <div ref={panelRef} style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '420px', background: 'white', zIndex: 201, boxShadow: '-8px 0 40px rgba(13,43,34,0.2)', display: 'flex', flexDirection: 'column', fontFamily: 'Outfit, sans-serif', animation: 'slideIn 0.22s ease-out' }}>
          <style>{`@keyframes slideIn { from { transform: translateX(100%); opacity:0 } to { transform: translateX(0); opacity:1 } }`}</style>
          <div style={{ background: 'linear-gradient(135deg, #0d2b22, #163d2f)', padding: '28px 24px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', position: 'relative' }}>
            <button onClick={() => setPanel(null)} style={{ position: 'absolute', top: '14px', right: '14px', width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>X</button>
            ...
            {med.verifie && <span style={{ background: 'rgba(46,181,146,0.2)', color: '#2eb592', fontSize: '0.65rem', fontWeight: 700, padding: '3px 10px', borderRadius: '50px', border: '1px solid rgba(46,181,146,0.3)' }}>Medecin verifie</span>}
            <div style={{ display: 'flex', gap: '6px', background: 'rgba(255,255,255,0.08)', padding: '4px', borderRadius: '10px', width: '100%' }}>
              {[{ id: 'profil', label: 'Profil' }, { id: 'rdv', label: 'Prendre RDV' }].map(t => (
                <button key={t.id} onClick={() => setPanel((prev: any) => ({ ...prev, _tab: t.id }))} style={{ flex: 1, padding: '7px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontSize: '0.78rem', fontWeight: 700, background: tab === t.id ? 'white' : 'none', color: tab === t.id ? '#0d2b22' : 'rgba(255,255,255,0.5)', transition: 'all 0.15s' }}>{t.label}</button>
              ))}
            </div>
          </div>
                ...
                <div style={{ background: '#faf8f4', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  {med.profil?.telephone && (
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                      ...
                      <div>
                        <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#a8a090', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '2px' }}>Telephone</div>
                        <div style={{ fontSize: '0.85rem', color: '#22816a', fontWeight: 600 }}>{med.profil.telephone}</div>
                      </div>
                    </div>
                  )}
                  {med.tarif && (
          ...
          {tab === 'profil' && (
            <div style={{ padding: '16px 24px', borderTop: '1px solid #f0ece2', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <button onClick={() => setPanel((prev: any) => ({ ...prev, _tab: 'rdv' }))} style={{ padding: '11px', borderRadius: '10px', background: '#22816a', color: 'white', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem', fontFamily: 'Outfit, sans-serif' }}>
                Prendre un rendez-vous
              </button>
              <button onClick={() => setPanel(null)} style={{ padding: '11px', borderRadius: '10px', background: '#f0ece2', color: '#0d2b22', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', fontFamily: 'Outfit, sans-serif' }}>
                Fermer
              </button>
            </div>
          )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#faf8f4', fontFamily: 'Outfit, sans-serif' }}>
      {renderPanel()}
      {cliniquePanel && <PanelClinique clinique={cliniquePanel} onClose={() => setCliniquePanel(null)} />}
      ...
      <div style={{ background: '#0d2b22', padding: '20px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          {user ? (
            <button onClick={() => router.push('/dashboard/patient')} style={{ padding: '8px 18px', borderRadius: '10px', background: '#22816a', color: 'white', border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 600, fontSize: '0.82rem' }}>
              Mon espace
            </button>
          ) : (
            <>
              <button onClick={() => router.push('/login')} style={{ padding: '8px 18px', borderRadius: '10px', border: '1.5px solid rgba(255,255,255,0.2)', background: 'none', color: 'white', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 600, fontSize: '0.82rem' }}>Se connecter</button>
              <button onClick={() => router.push('/login')} style={{ padding: '8px 18px', borderRadius: '10px', background: '#2eb592', color: '#0d2b22', border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: '0.82rem' }}>S inscrire</button>
            </>
          )}
        </div>
      </div>

      <div style={{ background: '#0d2b22', padding: '0 48px 36px' }}>
        <div style={{ background: 'white', borderRadius: '16px', padding: '8px', display: 'flex', gap: '8px', alignItems: 'center', boxShadow: '0 24px 64px rgba(0,0,0,0.2)', flexWrap: 'wrap' }}>
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && fetchAll(specialite, search)}
            placeholder="Médecin, clinique, imagerie, chirurgie…"
            style={{ flex: 1, minWidth: '160px', padding: '10px 14px', border: 'none', outline: 'none', fontFamily: 'Outfit, sans-serif', fontSize: '0.9rem', color: '#1a1512', background: 'none' }}
          />
          <select value={specialite} onChange={e => { setSpecialite(e.target.value); fetchAll(e.target.value, search) }} style={selectStyle}>
            {SPECIALITES.map(s => <option key={s}>{s}</option>)}
          </select>
          <button onClick={geolocalize} disabled={locLoading} style={{ padding: '10px 18px', borderRadius: '10px', background: userPos ? '#e8f5f1' : '#fdf8ec', color: userPos ? '#22816a' : '#c8992a', border: `1.5px solid ${userPos ? '#22816a' : '#c8992a'}`, cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
            {locLoading ? 'Localisation…' : userPos ? 'Position detectee' : 'Pres de moi'}
          </button>
          <button onClick={() => fetchAll(specialite, search)} style={{ padding: '10px 24px', borderRadius: '10px', background: '#22816a', color: 'white', border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: '0.88rem', whiteSpace: 'nowrap' }}>
            Rechercher
          </button>
        </div>
        ...
        {userPos && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '12px', flexWrap: 'wrap' }}>
            ...
            {[5, 10, 20, 50].map(r => (
              <button key={r} onClick={() => setRayon(r)} style={{ padding: '4px 14px', borderRadius: '50px', border: 'none', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 700, background: rayon === r ? '#2eb592' : 'rgba(255,255,255,0.1)', color: rayon === r ? '#0d2b22' : 'rgba(255,255,255,0.5)' }}>{r} km</button>
            ))}
            <button onClick={() => setShowMap(!showMap)} style={{ padding: '4px 14px', borderRadius: '50px', border: '1px solid rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 700, background: showMap ? 'rgba(255,255,255,0.15)' : 'none', color: 'rgba(255,255,255,0.6)', marginLeft: 'auto' }}>
      ...
      {showMap && mapReady && userPos !== null && (
        <div style={{ height: '380px', borderBottom: '1px solid #f0ece2' }}>
      ...
      <div style={{ padding: '32px 48px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.3rem', fontWeight: 600, color: '#0d2b22' }}>
            {loading ? 'Recherche en cours…' : totalResultats + ' résultat' + (totalResultats > 1 ? 's' : '') + ' trouvé' + (totalResultats > 1 ? 's' : '')}
          </div>
          <div style={{ display: 'flex', gap: '6px' }}>
            {([
              { id: 'tous', label: 'Tous', count: medecinsAfiltres.length + cliniquesAfiltres.length },
              { id: 'medecins', label: 'Medecins', count: medecinsAfiltres.length },
              { id: 'cliniques', label: 'Cliniques', count: cliniquesAfiltres.length },
            ] as const).map(f => (
              <button key={f.id} onClick={() => setTypeFiltre(f.id)} style={{ padding: '6px 14px', borderRadius: '50px', border: 'none', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, background: typeFiltre === f.id ? '#0d2b22' : '#f0ece2', color: typeFiltre === f.id ? 'white' : '#7a7260' }}>
                {f.label} ({f.count})
              </button>
            ))}
          </div>
          {userPos && (
            <button onClick={() => { setUserPos(null); setShowMap(false); setMapReady(false) }} style={{ padding: '6px 14px', borderRadius: '8px', background: '#f0ece2', border: 'none', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, color: '#7a7260' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px', background: 'white', borderRadius: '16px', border: '1px solid #f0ece2' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: '#a8a090', marginBottom: '16px' }}>
              Essayez d autres critères
            </div>
            {userPos && (
              <button onClick={() => setRayon(50)} style={{ padding: '10px 22px', borderRadius: '10px', background: '#22816a', color: 'white', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}>
                Elargir a 50 km
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
              ...
              <div>
                {typeFiltre === 'tous' && (
                  <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.1rem', fontWeight: 600, color: '#0d2b22', marginBottom: '14px', paddingBottom: '10px', borderBottom: '1px solid #f0ece2' }}>
                    Etablissements de sante ({cliniquesAfiltres.length})
                  </div>
                )}
            ...
            {(typeFiltre === 'tous' || typeFiltre === 'medecins') && medecinsAfiltres.length > 0 && (
              <div>
                {typeFiltre === 'tous' && (
                  <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.1rem', fontWeight: 600, color: '#0d2b22', marginBottom: '14px', paddingBottom: '10px', borderBottom: '1px solid #f0ece2' }}>
                    Medecins ({medecinsAfiltres.length})
                  </div>
                )}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
...
function CliniqueCard({ clinique, onOpen }: { clinique: any, onOpen: () => void }) {
  ...
  return (
    <div
      onClick={onOpen}
      style={{ background: 'white', borderRadius: '16px', border: '1px solid #f0ece2', overflow: 'hidden', cursor: 'pointer', transition: 'all 0.15s' }}
    ...
    >
      <div style={{ background: 'linear-gradient(135deg, #0d2b22, #1a4535)', padding: '22px', display: 'flex', alignItems: 'center', gap: '14px' }}>
        <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(200,153,42,0.2)', border: '2px solid rgba(200,153,42,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', flexShrink: 0 }}>🏥</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.1rem', fontWeight: 600, color: 'white' }}>{clinique.nom}</div>
          <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', marginTop: '3px' }}>{clinique.region || 'Etablissement de sante'}</div>
        </div>
      ...
      <div style={{ padding: '16px 20px' }}>
        {categories.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '12px' }}>
            {categories.slice(0, 4).map((cat: string) => (
              <span key={cat} style={{ fontSize: '0.72rem', padding: '3px 8px', borderRadius: '50px', background: '#e8f5f1', color: '#22816a', fontWeight: 600 }}>
                {ICONS[cat] || '⚕️'} {CATEGORIES_CLINIQUE.find(c => c.id === cat)?.label?.split(' ')[0]}
              </span>
            ))}
            {categories.length > 4 && (
              <span style={{ fontSize: '0.72rem', padding: '3px 8px', borderRadius: '50px', background: '#f0ece2', color: '#7a7260', fontWeight: 600 }}>
                +{categories.length - 4}
              </span>
            )}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '12px' }}>
          {clinique.adresse && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.8rem', color: '#7a7260' }}><span>📍</span>{clinique.adresse}</div>}
          {clinique.telephone && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.8rem', color: '#7a7260' }}><span>📞</span>{clinique.telephone}</div>}
          {clinique.distance !== undefined && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.8rem', color: '#22816a', fontWeight: 600 }}>
              <span>📍</span>
              {clinique.distance < 1 ? Math.round(clinique.distance * 1000) + 'm' : clinique.distance.toFixed(1) + 'km'}
            </div>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '0.75rem', color: '#a8a090' }}>Voir les services et reserver</span>
...
function MedecinCard({ med, onOpen }: { med: any, onOpen: () => void }) {
  return (
    <div
      ...
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = 'none'; (e.currentTarget as HTMLElement).style.transform = 'none' }}
    >
      <div style={{ background: 'linear-gradient(135deg, #0d2b22, #163d2f)', padding: '22px', display: 'flex', alignItems: 'center', gap: '14px' }}>
        <Avatar url={med.profil?.avatar_url} prenom={med.profil?.prenom} nom={med.profil?.nom} role="medecin" size={52} />
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.1rem', fontWeight: 600, color: 'white' }}>
              Dr. {med.profil?.prenom} {med.profil?.nom}
            </div>
            {med.verifie && (
              <span style={{ background: 'rgba(46,181,146,0.2)', color: '#2eb592', fontSize: '0.6rem', fontWeight: 700, padding: '2px 7px', borderRadius: '50px', border: '1px solid rgba(46,181,146,0.3)' }}>
                Verifie
              </span>
            )}
          </div>
          <div style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.55)', marginTop: '3px' }}>{med.specialite}</div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          {med.distance !== undefined && (
            <div style={{ background: 'rgba(46,181,146,0.2)', color: '#2eb592', fontSize: '0.72rem', fontWeight: 700, padding: '3px 8px', borderRadius: '50px', border: '1px solid rgba(46,181,146,0.2)', whiteSpace: 'nowrap' }}>
              {med.distance < 1 ? Math.round(med.distance * 1000) + 'm' : med.distance.toFixed(1) + 'km'}
            </div>
          )}
          {med.note_moyenne > 0 && (
            <div style={{ color: '#e6b84a', fontSize: '0.82rem', fontWeight: 700, marginTop: '4px' }}>
              ★ {med.note_moyenne}
            </div>
          )}
        </div>
      ...
      <div style={{ padding: '16px 20px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', marginBottom: '14px' }}>
          {med.adresse && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: '#7a7260' }}><span>📍</span>{med.adresse}</div>}
          {med.region && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.78rem', color: '#a8a090' }}><span>🗺</span>{med.region}</div>}
          {med.tarif && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: '#7a7260' }}><span>💰</span>{Number(med.tarif).toLocaleString()} Ar</div>}
          {med.profil?.telephone && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.82rem', color: '#7a7260' }}><span>📞</span>{med.profil.telephone}</div>}
        </div>
        {med.presentation && (
          <p style={{ fontSize: '0.8rem', color: '#7a7260', lineHeight: 1.6, marginBottom: '14px', borderTop: '1px solid #f0ece2', paddingTop: '12px', margin: '0 0 14px' }}>
            {med.presentation.slice(0, 120)}{med.presentation.length > 120 ? '…' : ''}
          </p>
        )}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: med.presentation ? '0' : '8px' }}>
          <span style={{ fontSize: '0.75rem', color: '#a8a090' }}>Voir le profil et prendre RDV</span>
          <span style={{ fontSize: '1rem', color: '#22816a', fontWeight: 700 }}>→</span>
        </div>
      </div>
    </div>
...
function RechercheRdvForm({ med, user, userProfil, supabase, router, onBooked }: any) {
  ...
  const book = async () => {
    ...
    const { error } = await supabase.from('rendez_vous').insert({
      medecin_id: med.id, patient_id: user.id,
      date_rdv: dateRdvISO, motif, statut: 'en_attente',
    })
    if (!error) {
      ...
      await supabase.from('notifications').insert({
        ...
        corps: patientNom + ' souhaite un RDV le ' + dateFormatee + ' a ' + selectedTime.replace(':', 'h') + (motif ? ' — Motif : ' + motif : '') + '.',
      })
      setBooked(true)
  ...
  if (booked) return (
    <div style={{ background: '#e8f5f1', borderRadius: '16px', padding: '32px', textAlign: 'center', marginTop: '8px' }}>
      <div style={{ fontSize: '2rem', marginBottom: '12px' }}>✅</div>
      <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.1rem', fontWeight: 600, color: '#0d2b22', marginBottom: '6px' }}>Demande envoyee !</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      {selectedDate && (
        <div>
          ...
          {!loadingCreneaux && creneaux.length === 0 && (
            <div style={{ padding: '12px', background: '#fdf8ec', borderRadius: '10px', fontSize: '0.82rem', color: '#c8992a', textAlign: 'center' }}>
      ...
      {!user && (
        <div style={{ background: '#fdf8ec', borderRadius: '10px', padding: '12px 14px', border: '1px solid rgba(200,153,42,0.2)', fontSize: '0.78rem', color: '#c8992a', textAlign: 'center' }}>
          ...
          <span onClick={() => router.push('/login')} style={{ fontWeight: 700, cursor: 'pointer', textDecoration: 'underline', marginLeft: '4px' }}>
            Se connecter
          </span>
        </div>
      )}
